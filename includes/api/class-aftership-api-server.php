...
class AfterShip_API_Server {
	 ...
	 *
	 * @var $handler WC_API_Handler handle.
	 */
	public $handler;
	...
	public function __construct( $path ) {
		...
		if ( empty( $path ) ) {
			$path = isset( $_SERVER['PATH_INFO'] ) ? wc_clean( wp_unslash( $_SERVER['PATH_INFO'] ) ) : '/';
		}

		$request_method = isset( $_SERVER['REQUEST_METHOD'] ) ? wc_clean( wp_unslash( $_SERVER['REQUEST_METHOD'] ) ) : '';
	...
	public function serve_request() {
		...
		/**
		 * This is a filter rather than an action, since this is designed to be re-entrant if needed.
		 *
		 * @since 2.1
		 */
	 ...
	 *
	 * @since 2.1
	 * @param WP_Query|WP_User_Query $query query obj.
	 */
	 ...
	 * @param bool       $convert_to_utc convert to utc.
