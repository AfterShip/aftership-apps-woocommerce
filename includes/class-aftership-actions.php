...
class AfterShip_Actions
{
    ...
    public function display_tracking_info($order_id)
    {
        wc_get_template('myaccount/view-order.php',
            ...
            'aftership-woocommerce-tracking/', aftership()->get_plugin_path() . '/templates/');
    }
    ...
    public function add_tracking_item($order_id, $args)
    {
        ...
        $tracking_item['additional_fields'] = [
        ...
        foreach ($tracking_items as $item) {
            if ($item['tracking_id'] == $tracking_item['tracking_id']) $exist = true;
        }
        if (!$exist) $tracking_items[] = $tracking_item;
 ...
 */
    ...
    {
        if (!current_user_can('manage_aftership')) return;
